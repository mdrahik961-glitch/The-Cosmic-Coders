...
# Replace old button with open overlay button
$c = $c.Replace(
@'
              <button
...
# Also replace plain anchor if still present
$c = [regex]::Replace(
  $c,
...
# Add fullscreen overlay only if missing
if ($c -notmatch 'src="https://eyes.nasa.gov/apps/orrery/"') {
  $overlay = @'
      {showUniverseEmbed && (
        <div style={styles.universeOverlay}>
          ...
          <div style={styles.universeFallbackBar}>
            If NASA blocks embedding in your browser, use this:
            <a
...
Write-Host "Safe 3D tab update applied."
